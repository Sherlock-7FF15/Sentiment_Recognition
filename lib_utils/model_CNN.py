...
class LstmTFModel:
    ...
    def initialize_model(self):
        ...
        # [128,2]
        self.labels = tf.placeholder(tf.float32, [128,self.numClasses], name="labels")
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope('conv-maxpool-%s' % filter_size) as scope:
                ...
                conv = tf.layers.conv2d(embedding_inputs_expanded, filters=self.num_filters, kernel_size=[filter_size, self.embedding_size],
        ...
        with tf.name_scope('fc1'):
            ...
            # add_layer_1 = tf.add(igenout,igenout)
            add_layer_1 = layer_1
        ...
        with tf.name_scope('fc2'):
            ...
            h2 = self.variable_on_cpu('h2', [self.n_hidden_1, self.n_hidden_2], tf.random_normal_initializer(stddev=self.h_stddev))
        ...
        with tf.name_scope('fc3'):
            b3 = self.variable_on_cpu('b3', [self.n_hidden_3], tf.random_normal_initializer(stddev=self.b_stddev))
            h3 = self.variable_on_cpu('h3', [self.n_hidden_2, self.n_hidden_3], tf.random_normal_initializer(stddev=self.h_stddev))
        ...
        with tf.name_scope('fc4'):
            ...
            h4 = self.variable_on_cpu('h4', [400, 192],
                                      tf.random_normal_initializer(stddev=self.h_stddev))
            layer_4 = tf.minimum(tf.nn.relu(tf.add(tf.matmul(layer_3, h4), b4)), self.relu_clip)
            layer_4 = tf.nn.dropout(layer_4, self.keep_dropout)
            print('没变形的layer_4', layer_4.shape)
            self.output = tf.reshape(layer_4,[self.batchSize,self.ksize,self.strides,self.numClasses])

        # Fully connected layer
        with tf.name_scope('fc6'):
            # 全连接层用于softmax分类
            nt_hpool = self.avg_pool_16x16(self.output)
            nt_hpool_flat = tf.reshape(nt_hpool,[self.batchSize,self.numClasses])
