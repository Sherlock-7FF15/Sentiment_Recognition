...
class LstmTFModel:
    ...
    def initialize_model(self):
        ...
        with tf.name_scope('dropout'):
            ...
            conv_output = tf.contrib.layers.dropout(pooled_concat_flat, keep_prob=self.keep_prob,
                                                          is_training=self.is_training)
            print('卷积dropout后输出', conv_output)
        # 定义堆叠多层双向RNN
        ...
        with tf.name_scope('lstm'):
            ...
            layer_rnn = tf.reshape(conv_output, [self.batchSize, -1, 384])
            print('layer_rnn的形状', layer_rnn.shape)
            outputs, _, _ = rnn.stack_bidirectional_dynamic_rnn([mcell], [mcell_bw], layer_rnn, dtype=tf.float32)
            ...
            rnn_output = tf.reshape(self.output, [self.batchSize, -1, 256])  # (128,2,256)
        # Define Forward RNN Cell
        # 加入attention，注意力机制
        with tf.variable_scope('attention'):
            cnn_attention_output = self.addAttentionToModel(rnn_output)  # self.output=(128,128)
