...
class LstmTFModel:
    ...
    def initialize_filepaths(self):
        ...
        self.lstm_model_path = "./model/pretrained_lstm_tf.ckpt"
    ...
    def initialize_model(self):
        ...
        # [128,200]
        self.cnn_input_data = tf.placeholder(tf.int32, [128, self.maxSeqLength],
                                             name="cnn_input_data")
        # [128,3]
        ...
        print('embed0：', self.embed.get_shape())  # embed0 (128, 200, 300)
        # 作用域
        ...
        #     rnn_attention_output = self.rnnAttentionToModel(self.output)  # self.output=(128,128)
        #     print('attention输出的形状', rnn_attention_output.shape)  # (128,256)
        #
        # with tf.name_scope('dropout'):
        #     # pooled_concat_flat shape: (batch_size, num_filters * len(filter_sizes))
        #     rnn_attention_output = tf.contrib.layers.dropout(rnn_attention_output, keep_prob=self.keep_prob,
        #                                                  is_training=self.is_training)
        # 定义CNN
        # 这里的embed不需要变形
        #  样本数量，图片大小，过滤器的通道数
        embedding_inputs_expanded = tf.reshape(self.output, [128,256,1,1])
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope('conv-maxpool-%s' % filter_size) as scope:
                ...
                # (3,256,1,128)(4,256,1,128)(5,256,1,128)
                # [128,256,1,1]
                ...
                # N = (W-F+2P)/S+1
                print('第二种模型',conv)  #(128, 254, 1, 128) (128, 253, 1, 128) (128, 252, 1, 128)
                # 最大池化，将[254,1]的图片变为[1,1]  三个[128,1,1,128]
        ...
        with tf.name_scope('dropout'):
            ...
            layer_3 = tf.reshape(self.final_output,[128,1,384])
            # 注意力接收三个参数
        ...
        with tf.name_scope('fc4'):
            ...
            h4 = self.variable_on_cpu('h4', [384, 192],
        ...
        with tf.name_scope('loss'):
            #  不是GAN的损失
            cross_entropy = tf.nn.softmax_cross_entropy_with_logits_v2(logits=nt_hpool_flat,
                                                                       labels=self.labels)
    ...
    def train_epochs(self, n_epochs):
        ...
        num = 0
        while num < n_epochs:
            print("Epoch " + str(num + 1) + ":BIGRU_CNN_Att\n")
